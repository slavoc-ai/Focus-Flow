 ...
 * - Enhanced sub_tasks table with title, action, details fields
 *
 * =================================================================
 */
...
-- =================================================================
-- 4. TABLE: sub_tasks (Enhanced with title, action, details)
-- =================================================================

-- Create enhanced sub_tasks table
CREATE TABLE IF NOT EXISTS public.sub_tasks (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id uuid NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  title text DEFAULT '', -- NEW: Short headline for the task
  action text DEFAULT '', -- NEW: Immediate call to action
  details text DEFAULT '', -- NEW: Longer explanation with context
  description text NOT NULL, -- Keep for backward compatibility
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'sub_tasks' AND column_name = 'title'
  ) THEN
    ALTER TABLE public.sub_tasks ADD COLUMN title text DEFAULT '';
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'sub_tasks' AND column_name = 'action'
  ) THEN
    ALTER TABLE public.sub_tasks ADD COLUMN action text DEFAULT '';
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'sub_tasks' AND column_name = 'details'
  ) THEN
    ALTER TABLE public.sub_tasks ADD COLUMN details text DEFAULT '';
  END IF;
END $$;
